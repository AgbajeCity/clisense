...
model = scaler = le_s = le_se = le_z = le_l = None


def load_model():
    global model, scaler, le_s, le_se, le_z, le_l
    try:
        ...
        le_l = joblib.load(BASE / "models" / "clisense_le_label.pkl")
    except Exception:
        ...
        labels = [
            "Flood Risk" if r7 > 150 and h > 80
            else "Drought Risk" if r < 2 and r30 < 20 and h < 40
            else "Normal"
        ...
        X = np.column_stack([
            rainfall, temp, humidity, np.full(n, 2.5), rain_7d, rain_30d,
        ...
        joblib.dump(le_l, BASE / "models" / "clisense_le_label.pkl")
...
def predict(req: PredictionRequest):
    ...
    doy = req.month * 30
    try:
        s_enc = le_s.transform([req.state])[0]
    except Exception:
        s_enc = 0
    try:
    ...
    try:
        ...
        z_enc = 0
    feat = np.array([[
        req.rainfall_mm, req.temp_c, req.humidity_pct, 2.5,
    ...
    return {
        "state": req.state, "month": req.month, "prediction": label,
        "confidence": round(float(proba[pred_idx]), 4),
        "probabilities": dict(zip(le_l.classes_, map(float, proba))),
        "recommendation": rec
    }
